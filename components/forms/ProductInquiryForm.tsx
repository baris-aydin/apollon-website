...
const t = {
  tr: {
    name: "Ad Soyad", phone: "Telefon", email: "E-posta", product: "İlgilenilen Ürün",
    vehicleBrand: "Araç Markası", vehicleModel: "Araç Modeli", vehicleYear: "Model Yılı",
    message: "Mesaj", submit: "Bilgi Talep Et",
    success: "Talebiniz alındı. En kısa sürede dönüş yapacağız.",
    error: "Bir hata oluştu. Lütfen tekrar deneyin.",
  },
  en: {
    name: "Full Name", phone: "Phone", email: "Email", product: "Product of Interest",
    vehicleBrand: "Vehicle Brand", vehicleModel: "Vehicle Model", vehicleYear: "Year",
    message: "Message", submit: "Request Information",
...
export function ProductInquiryForm({ locale, preselectedProduct = "" }: Props) {
  ...
  const [form, setForm] = useState({
    name: "", phone: "", email: "", product: preselectedProduct,
    vehicleBrand: "", vehicleModel: "", vehicleYear: "", message: "",
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus("loading")
    try {
      const res = await fetch("/api/product-inquiry", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
      if (res.ok) { setStatus("success"); setForm({ name: "", phone: "", email: "", product: "", vehicleBrand: "", vehicleModel: "", vehicleYear: "", message: "" }) }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <input type="text" value={form.product} onChange={set("product")} className={inputClass} />
      </div>
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="space-y-1.5">
          <label className={labelClass}>{l.vehicleBrand}</label>
          <input type="text" value={form.vehicleBrand} onChange={set("vehicleBrand")} className={inputClass} />
        </div>
        <div className="space-y-1.5">
          <label className={labelClass}>{l.vehicleModel}</label>
          <input type="text" value={form.vehicleModel} onChange={set("vehicleModel")} className={inputClass} />
        </div>
        <div className="space-y-1.5">
          <label className={labelClass}>{l.vehicleYear}</label>
          <input type="text" placeholder="2022" value={form.vehicleYear} onChange={set("vehicleYear")} className={inputClass} />
        </div>
      </div>
      <div className="space-y-1.5">
